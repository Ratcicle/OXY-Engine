...
impl Editor {
    pub(super) fn library(&mut self, ctx: &egui::Context) {
        egui::TopBottomPanel::bottom("library").resizable(true).default_height(145.).height_range(90.0..=360.0).show(ctx,|ui| {
            ui.horizontal_wrapped(|ui|{ui.strong("BIBLIOTECA DO PROJETO");ui.add(egui::TextEdit::singleline(&mut self.asset_search).hint_text("Filtrar recursos…").desired_width(170.));if ui.button("Importar PNG").clicked(){self.import(AssetKind::Texture);}
            ...
            egui::ScrollArea::vertical().show(ui,|ui|{
                ui.horizontal_wrapped(|ui|{
                    for asset in assets.iter().filter(|a|a.name.to_lowercase().contains(&asset_filter)){
                        let card = ui.group(|ui|{ui.vertical(|ui|{ui.set_width(200.);ui.strong(&asset.name);ui.small(match asset.kind{AssetKind::Texture=>"PNG · textura",AssetKind::Audio=>"WAV · áudio",AssetKind::Model=>"Modelo · peças editáveis"});
                            match asset.kind {
                                ...
                                AssetKind::Texture=>{if ui.button("Criar sprite / aplicar").clicked(){self.ensure_texture(&asset.id);if self.selected.is_none(){self.add_entity(Some(Primitive::Sprite),&asset.name);}
...
if let Some(p)=self.state.images.get(&asset.id){let ratio=p.width as f32/p.height as f32;if let Some(e)=self.scene_mut().entity_mut(&id)&& e.primitive==Some(Primitive::Sprite){e.dimensions[0]=e.dimensions[1]*ratio;}}}}},
                            ...
                            if ui.small_button("Excluir recurso do projeto").clicked() { self.delete_asset=Some(asset.id.clone()); }
                        });});
                        if self.locate_asset.as_ref()==Some(&asset.id) { card.response.scroll_to_me(Some(egui::Align::Center)); ui.painter().rect_stroke(card.response.rect,3.,egui::Stroke::new(2.,Color32::GOLD),egui::StrokeKind::Inside); self.locate_asset=None; }
                    }
