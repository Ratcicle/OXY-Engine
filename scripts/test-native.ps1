...
if (!$GraphicsOnly) {
    $checks += @(
        @('oxy_editor','native_body_offset_v0331'),
...
$checks += @(
    ...
    @('oxy_render','native_gpu_component_overlay_depth_and_cache')
)
$started = !$From
$completed = 0
foreach ($check in $checks) {
    ...
    & (Join-Path $PSScriptRoot 'cargo.ps1') @arguments 2>&1 | Tee-Object -FilePath (Join-Path $workspace "artifacts/v021-final-$test.log")
